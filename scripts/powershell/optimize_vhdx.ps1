...
function Compress-Vhdx {
    ...
    #>
        ...
        (
            ...
            [Parameter(ValueFromPipelineByPropertyName,Position=0)]$Path = $env:LOCALAPPDATA,
    
            # Go through subfolders, if selected
            [Parameter(Position=1)][switch]$IncludeSubfolders,

            [Parameter(Position=2)][int]$Depth = 3
        )
        ...
        Begin {
            ...
            if ($IncludeSubfolders) {
                $AllVhdx = Get-ChildItem *.vhdx -Path $Path -Recurse -Depth $Depth -ErrorAction SilentlyContinue
                }
            else {
